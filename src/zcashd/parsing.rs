...
pub fn parse_compact_size(parser: &mut Parser) -> Result<usize> {
    match parse!(parser, u8, "compact size")? {
        0xfd =>
            parse!(parser, u16, "compact size").map(|n| n as usize),
        0xfe =>
            parse!(parser, u32, "compact size").map(|n| n as usize),
        0xff =>
            parse!(parser, u64, "compact size").map(|n| n as usize),
        size => Ok(size as usize),
    }
}

pub fn parse_pair<T: Parse, U: Parse>(parser: &mut Parser) -> Result<(T, U)> {
    let first = parse!(parser, "first item of pair")?;
    let second = parse!(parser, "second item of pair")?;
    Ok((first, second))
}
...
pub fn parse_fixed_length_vec<T: Parse>(parser: &mut Parser, length: usize) -> Result<Vec<T>> {
    ...
    for i in 0..length {
        items.push(parse!(parser, format!("array item {} of {}", i, length - 1))?);
    }
    Ok(items)
}
...
pub fn parse_vec<T: Parse>(parser: &mut Parser) -> Result<Vec<T>> {
    let length = parse_compact_size(parser).context("array length")?;
...
pub fn parse_map<K: Parse, V: Parse>(parser: &mut Parser) -> Result<Vec<(K, V)>> {
    let length = parse_compact_size(parser).context("map length")?;
    let mut items = Vec::with_capacity(length);
    for _ in 0..length {
        items.push(parse_pair::<K, V>(parser).context("map item")?);
    }
    Ok(items)
}
...
pub fn parse_optional<T: Parse>(parser: &mut Parser) -> Result<Option<T>> {
    match parse!(parser, u8, "optional discriminant")? {
        0x00 => Ok(None),
        0x01 => Ok(Some(parse!(parser, "optional value")?)),
