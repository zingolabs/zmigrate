...
use super::{BlockData, ZingoWallet};
...
impl<'a> ZingoParser<'a> {
    ...
    pub fn parse(&mut self) -> Result<ZingoWallet> {
        self.parse_with_param(ZingoConfig)
    }
    ...
    pub fn parse_with_param(&mut self, config: ZingoConfig) -> Result<ZingoWallet> {
        ...
        let wallet_capability = parse!(p, "wallet_capability")?;

        let mut blocks = parse!(p, Vec<BlockData>, "blocks")?;
